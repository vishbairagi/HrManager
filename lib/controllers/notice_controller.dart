...
class NoticeController extends GetxController {
  ...
  /// API URL
  final String baseUrl = "https://wordpress.thehrmanagers.com/wp-json/hr/user/user_notice";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          notices.value = jsonResponse['data']; // Only take the data array
        } else {
          Get.snackbar("Error", jsonResponse['message'] ?? "Failed to load notices");
        }

      } else {
        Get.snackbar("Error", "Failed to load notices: ${response.statusCode}");
      }
    } catch (e) {
